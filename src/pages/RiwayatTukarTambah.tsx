...
/*  Component                                                          */
/* ------------------------------------------------------------------ */
export default function RiwayatTukarTambah() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getTransactionsWithStockDetailsByType('Tukar Tambah');
  ...
  const totalTT = filteredTransactions.length;
    ...
    <div className="pb-12">
      ...
      >
        <h1 className="font-display text-[36px] sm:text-[40px] text-slate-900 leading-tight">
          Riwayat Tukar Tambah
        </h1>
        <p className="text-[13px] text-slate-500 mt-1">
          {totalTT} transaksi · {formatRupiah(totalSelisih)}
        </p>
      </motion.div>
      ...
      >
        <div className="bg-white rounded-2xl border border-slate-200 shadow-card p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-[0.08em]">Total TT</p>
          </div>
          <p className="font-mono text-[32px] font-bold text-slate-900 leading-none">{totalTT}</p>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-card p-6">
          <div className="flex items-center gap-2 mb-2">
        ...
        <div className="space-y-6">
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="flex flex-col sm:items-end gap-2">
                          <p className="text-[10px] text-slate-400 uppercase tracking-wider">Selisih Bayar</p>
          ...
          {transactions.length === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
              ...
              <p className="text-[15px] font-medium text-slate-500">Belum ada transaksi tukar tambah</p>
              <p className="text-[12px] text-slate-400 mt-1">
                Transaksi tukar tambah akan muncul di sini setelah dicatat
              </p>
            </motion.div>
          )}
