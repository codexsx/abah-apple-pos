...
begin
  ...
  loop
    ...
    v_status := coalesce(nullif(v_item->>'status', ''), 'READY');
    ...
    if not (v_status = any(array['READY','SERVIS','KANIBAL','RUSAK'])) then
      raise exception 'Invalid purchase stock status: %', v_status;
    end if;
