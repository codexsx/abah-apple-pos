-- Always rank the leaderboard by completed previous-month units first.
-- This corrects deployments that retained an earlier function without ORDER BY.
...
begin
  ...
  with bounds as (
    select
      (date_trunc('month', now() at time zone 'Asia/Jakarta') at time zone 'Asia/Jakarta') as current_month_start,
      ((date_trunc('month', now() at time zone 'Asia/Jakarta') - interval '1 month') at time zone 'Asia/Jakarta') as previous_month_start,
